...
# Program untuk men-generate pelat nomor berdasarkan domisili (Keresidenan Banyumas) & jenis kendaraan 
...
def generator(userDomisili, userTipe):
    ...
    arrayBanyumas = ["A","E","G","H","J","R","S"]
    arrayCilacap = ["B","F","K","N","P","T"]
    arrayPurbalingga = ["C","L","Q","U","V","Z"]
    arrayBanjarnegara = ["D","I","M","O","W","Y",]

    match userDomisili:
        case 1: kodePelat = random.choice(arrayBanyumas)
        case 2: kodePelat = random.choice(arrayCilacap)
        case 3: kodePelat = random.choice(arrayPurbalingga)
        case 4: kodePelat = random.choice(arrayBanjarnegara)
    ...
    match kodePelat:
        ...
        case "B" | "C" | "D" :
            ...
            if angkaNentuinBelakang == 1:
                ...
                print(f"R {str(angkaAcak)} {PelatHuruf1}{PelatHuruf2}{kodePelat}")
            else:
                PelatHuruf2 = random.choice(alfabet)
                print(f"R {str(angkaAcak)}  {PelatHuruf2}{kodePelat}")
        case _:
            PelatHuruf2 = random.choice(alfabet)
            print(f"R {str(angkaAcak)}  {PelatHuruf2}{kodePelat}")
